...
class Camera extends Homey.Device {
    ...
    onAudioDetection(payload, actionType = null, eventId = null) {
        ...
        this.homey.app.debug('[Audio] onAudioDetection ' + JSON.stringify(event));
        ...
        if (audioDetectTypes && audioDetectTypes.length > 0) {
            ...
            for (let audioDetectionType of audioDetectTypes) {
                this.homey.app.debug(`audio detection event on Camera ${this.getData().id}, with type ${audioDetectionType}`);
            ...
            this.homey.app.debug(`audio detection event on Camera ${this.getData().id}, with unspecified type (any)`);
